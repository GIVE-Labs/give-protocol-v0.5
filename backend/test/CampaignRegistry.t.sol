...
contract CampaignRegistryTest is Test {
    ...
    function testSubmitCampaignStoresConfig() public {
        ...
        assertEq(cfg.initialDeposit, 0.005 ether);
        assertTrue(cfg.exists);
    }

    function testSubmitCampaignRevertsWithInsufficientDeposit() public {
        CampaignRegistry.CampaignInput memory input = CampaignRegistry
            .CampaignInput({
                id: campaignId,
                payoutRecipient: makeAddr("payout"),
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CampaignRegistry.InsufficientSubmissionDeposit.selector,
                0.005 ether,
                0.001 ether
            )
        );
        campaignRegistry.submitCampaign{value: 0.001 ether}(input);
    }
