...
contract AaveAdapter is IYieldAdapter, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        if (_asset == address(0) || _vault == address(0) || 
            _aavePool == address(0) || _admin == address(0)) {
     ...
     */
    function divest(uint256 assets) 
        external 
        override 
        onlyVault 
        nonReentrant 
     ...
     */
    function setEmergencyExitBps(uint256 _bps) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
     ...
     */
    function getAaveInfo() external view returns (
        address poolAddress,
        address aTokenAddress,
