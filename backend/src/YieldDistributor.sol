...
contract YieldDistributor is ReentrancyGuard, Pausable, Ownable {
    ...
    function initiateDistribution() external nonReentrant whenNotPaused {
        if (!authorizedDistributors[msg.sender] && block.timestamp < lastDistributionTime + distributionInterval) {
     ...
     */
    function getDistributionRound(uint256 _round)
        external
        view
        returns (uint256 roundNumber, uint256 totalYieldDistributed, uint256 distributionTime, uint256 stakersCount)
    {
        DistributionRound storage round = distributionRounds[_round];
        return (round.roundNumber, round.totalYieldDistributed, round.distributionTime, round.stakersCount);
    }
     ...
     */
    function getNGOYieldForRound(uint256 _round, address _token, address _ngo) external view returns (uint256) {
    ...
    function getDistributionStatus()
        external
        view
        returns (bool canDistribute, uint256 timeUntilNextDistribution, uint256 totalSupportedTokens)
    {
