...
contract DonationRouter is AccessControl, ReentrancyGuard, Pausable {
    ...
    function distribute(
        address asset,
        uint256 amount
    ) external nonReentrant whenNotPaused returns (uint256 netDonation, uint256 feeAmount) {
     ...
     * @return feeAmount The fee amount collected
     */
    function distributeToMultiple(
        address asset,
        uint256 amount,
        address[] calldata ngos
    ) external nonReentrant whenNotPaused returns (uint256 totalNetDonation, uint256 feeAmount) {
        if (!authorizedCallers[msg.sender]) {
            revert Errors.UnauthorizedCaller();
        }
        if (asset == address(0)) revert Errors.ZeroAddress();
     ...
     */
    function getDistributionStats(address asset) external view returns (
        ...
        return (
            totalDonated[asset],
            totalFeeCollected[asset],
            ngoRegistry.getCurrentNGO(),
            feeBps
        );
    }
