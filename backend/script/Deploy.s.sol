...
contract Deploy is Script {
    ...
    function run() external returns (Deployed memory out) {
        ...
        // Use environment variables if available, otherwise use HelperConfig
        ...
        NGORegistry registry = new NGORegistry(deployer); // Use actual deployer wallet as admin
        DonationRouter router = new DonationRouter(deployer, address(registry), feeRecipient, admin, feeBps); // Use deployer as admin for role management

        GiveVault4626 vault = new GiveVault4626(IERC20(assetAddress), assetName, assetSymbol, deployer);
        StrategyManager manager = new StrategyManager(address(vault), deployer);
        
        // Use MockYieldAdapter for Anvil (chainid 31337), AaveAdapter for other networks
        IYieldAdapter adapter;
        if (block.chainid == 31337) {
            adapter = new MockYieldAdapter(assetAddress, address(vault), deployer);
            console.log("Using MockYieldAdapter for local testing");
        } else {
            adapter = new AaveAdapter(assetAddress, address(vault), aavePool, deployer);
            console.log("Using AaveAdapter for live network");
        }

        // Wire roles & params
        console.log("Deployer address:", deployer);
        console.log("Has DEFAULT_ADMIN_ROLE:", registry.hasRole(registry.DEFAULT_ADMIN_ROLE(), deployer));
